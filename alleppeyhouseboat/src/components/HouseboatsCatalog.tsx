...
interface HouseboatsCatalogProps {
  packages: Package[];
...
export default function HouseboatsCatalog({ packages, initialCategory = "all", videoSrc }: HouseboatsCatalogProps) {
  const [activeFilter, setActiveFilter] = useState(initialCategory.toLowerCase());

  // Filter houseboats
  const filteredPackages = packages.filter((pkg) => {
    if (!pkg.category.startsWith("houseboat-")) return false;
    if (activeFilter === "all") return true;
    return pkg.category === `houseboat-${activeFilter}`;
  });
    ...
    <div className="min-h-screen bg-bg-cream pb-20">
        ...
        <div className="relative w-full min-h-[320px] rounded-[2.5rem] overflow-hidden bg-primary-dark shadow-md border border-primary/5 flex flex-col justify-center py-16 text-center">
          ...
          <div className="max-w-4xl mx-auto px-6 relative z-20">
            <h1 className="font-serif text-3xl md:text-5xl text-white font-bold mb-4 drop-shadow-sm">
              Our Houseboat Collection
            </h1>
            <p className="font-sans text-xs md:text-sm text-white/80 max-w-xl mx-auto drop-shadow-sm leading-relaxed">
              Choose from a wide variety of premium and budget-friendly houseboats cruising through Alappuzha.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-12 mb-8">
        <div className="flex items-center justify-center">
          <div className="flex flex-wrap items-center justify-center gap-2 p-1.5 rounded-3xl bg-white shadow-sm border border-primary/5 max-w-2xl">
            {categories.map((cat) => {
            ...
            {filteredPackages.length > 0 ? (
              ...
              <div className="col-span-full py-16 text-center">
                <p className="font-sans text-base text-charcoal/60 mb-4">
                  No houseboat packages found in this category.
                </p>
                <a
                  href={`https://wa.me/917356462150?text=${encodeURIComponent(
                    "Hi! I want to enquire about houseboat packages in Alleppey."
                  )}`}
                  target="_blank"
